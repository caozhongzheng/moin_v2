 ...
 * 用户注册的服务器接口
 */
public class UserRegisterProtocal extends HttpPostProtocal implements HttpObserver {
    private static ILogger MyLog = LoggerFactory.getLogger(LoginModule.MODULE_NAME);

    // ===========================================================
    // Constructors
    // ===========================================================
    public UserRegisterProtocal(String url, Map data, Object tag) {
        super(url, data);
        setTag(tag);
    }
    ...
    public void onHttpResult(int resultCode, String respone) {
        MyLog.i("UserRegisterProtocal.onHttpResult()... resultCode=" + resultCode + ", response=" + respone);
        if (resultCode == AbsHttpOperation.ERROR_NONE) {
            EventBus.getDefault().post(new GetRegisterSuccessEvent(respone, getTag()));
        } else {
    ...
    // inner class
    // ===========================================================
    public static class GetRegisterSuccessEvent extends AbsProtocolEvent {
        private String mResponse;

        public GetRegisterSuccessEvent(String resp, Object tag) {
            setTag(tag);
            mResponse = resp;
        }
