 ...
 * 获取邮箱验证码的服务器接口
 */
public class UserGetEmailCodeProtocal extends HttpPostProtocal implements HttpObserver {
    private static ILogger MyLog = LoggerFactory.getLogger(LoginModule.MODULE_NAME);

    // ===========================================================
    // Constructors
    // ===========================================================
    public UserGetEmailCodeProtocal(String url, Map data, Object tag) {
        super(url, data);
        setTag(tag);
    }
    ...
    public void onHttpResult(int resultCode, String respone) {
        MyLog.i("UserGetEmailCodeProtocal.onHttpResult()... resultCode=" + resultCode + ", response=" + respone);
        if (resultCode == AbsHttpOperation.ERROR_NONE) {
            EventBus.getDefault().post(new GetEmailCodeSuccessEvent(respone, getTag()));
        } else {
    ...
    // inner class
    // ===========================================================
    public static class GetEmailCodeSuccessEvent extends AbsProtocolEvent {
        private String mResponse;

        public GetEmailCodeSuccessEvent(String resp, Object tag) {
            setTag(tag);
            mResponse = resp;
        }
