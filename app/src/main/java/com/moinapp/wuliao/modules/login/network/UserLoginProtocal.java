 ...
 * 用户登录的服务器接口
 */
public class UserLoginProtocal extends HttpPostProtocal implements HttpObserver {
    private static ILogger MyLog = LoggerFactory.getLogger(LoginModule.MODULE_NAME);

    // ===========================================================
    // Constructors
    // ===========================================================
    public UserLoginProtocal(String url, Map data, Object tag) {
        super(url, data);
        setTag(tag);
    }
    ...
    public void onHttpResult(int resultCode, String respone) {
        MyLog.i("UserLoginProtocal.onHttpResult()... resultCode=" + resultCode + ", response=" + respone);
        if (resultCode == AbsHttpOperation.ERROR_NONE) {
            EventBus.getDefault().post(new GetLoginSuccessEvent(respone, getTag()));
        } else {
    ...
    // inner class
    // ===========================================================
    public static class GetLoginSuccessEvent extends AbsProtocolEvent {
        private String mResponse;

        public GetLoginSuccessEvent(String resp, Object tag) {
            setTag(tag);
            mResponse = resp;
        }
