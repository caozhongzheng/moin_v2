...
*/
public class GetMyReplyProtocal extends HttpPostProtocal implements HttpObserver {
    private static ILogger MyLog = LoggerFactory.getLogger(MineModule.MODULE_NAME);

    // ===========================================================
    // Constructors
    // ===========================================================
    public GetMyReplyProtocal(String url, Object tag) {
        super(url, null);
        setTag(tag);
    }
    ...
    public void onHttpResult(int resultCode, String respone) {
        MyLog.i("GetMyReplyProtocal.onHttpResult()... resultCode=" + resultCode + ", response=" + respone);
        if (resultCode == AbsHttpOperation.ERROR_NONE) {
            EventBus.getDefault().post(new GetMyReplySuccessEvent(respone, getTag()));
        } else {
            Object object = getTag();
            if (object != null) {
                if (resultCode == NetConstants.NONE_NETWORK) {
                    ((MyReplyListener) object).onNoNetwork();
                } else {
                    ((MyReplyListener) object).onErr(resultCode);
                }
            }
        }
    }
    ...
    // inner class
    // ===========================================================
    public static class GetMyReplySuccessEvent extends AbsProtocolEvent {
        private String mResponse;

        public GetMyReplySuccessEvent(String resp, Object tag) {
            setTag(tag);
            mResponse = resp;
        }
     ...
     * 服务器返回的我参与的帖子列表
     */
    public static class GetMyReplyResponse extends BaseResponse {
        private List<PostInfo> replylist;

        public List<PostInfo> getReplylist() {
            return replylist;
        }

        public void setReplylist(List<PostInfo> replylist) {
