...
*/
public class DelCosplayProtocal extends HttpPostProtocal implements HttpObserver {
    private static ILogger MyLog = LoggerFactory.getLogger(MineModule.MODULE_NAME);

    // ===========================================================
    // Constructors
    // ===========================================================
    public DelCosplayProtocal(String url, Map data, Object tag) {
        super(url, data);
        setTag(tag);
    }
    ...
    public void onHttpResult(int resultCode, String respone) {
        MyLog.i("DelCosplayProtocal.onHttpResult()... resultCode=" + resultCode + ", response=" + respone);
        if (resultCode == AbsHttpOperation.ERROR_NONE) {
            EventBus.getDefault().post(new DelCosplaySuccessEvent(respone, getTag()));
        } else {
    ...
    // inner class
    // ===========================================================
    public static class DelCosplaySuccessEvent extends AbsProtocolEvent {
        private String mResponse;

        public DelCosplaySuccessEvent(String resp, Object tag) {
            setTag(tag);
            mResponse = resp;
        }
    ...
    /**
     * 服务器返回的删除大咖秀的response
     */
    public static class DelCosplayResponse extends BaseResponse {
