 ...
 * 用户数据更新的服务器接口
 */
public class UserUpdatePasswordProtocal extends HttpPostProtocal implements HttpObserver {
    private static ILogger MyLog = LoggerFactory.getLogger(LoginModule.MODULE_NAME);

    // ===========================================================
    // Constructors
    // ===========================================================
    public UserUpdatePasswordProtocal(String url, Map data, Object tag) {
        super(url, data);
        setTag(tag);
    }
    ...
    public void onHttpResult(int resultCode, String respone) {
        MyLog.i("GetUpdatePasswordSuccessEvent.onHttpResult()... resultCode=" + resultCode + ", response=" + respone);
        if (resultCode == AbsHttpOperation.ERROR_NONE) {
            EventBus.getDefault().post(new GetUpdatePasswordSuccessEvent(respone, getTag()));
        } else {
    ...
    // inner class
    // ===========================================================
    public static class GetUpdatePasswordSuccessEvent extends AbsProtocolEvent {
        private String mResponse;

        public GetUpdatePasswordSuccessEvent(String resp, Object tag) {
            setTag(tag);
            mResponse = resp;
        }
