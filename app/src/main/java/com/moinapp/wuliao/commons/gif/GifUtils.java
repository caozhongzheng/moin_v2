...
//
    ...
    /**
     * 这个方法是用common/gif/目录下的java层代码decode并显示gif的，会有oom情况出现
     * @param imageView
     * @param gifFile
     */
    public static void displayGif(ImageView imageView, String gifFile) {
        try {
            InputStream inputStream = new BufferedInputStream(new FileInputStream(gifFile));
            if (inputStream != null) {
            }
            GifAnimationDrawable gif = new GifAnimationDrawable(inputStream);
            if (gif != null) {
...
//        GifDrawable gifFromPath;
    ...
    /**
     */
    public static void recycleGif(ImageView imageView) {
        GifDrawable drawable = (GifDrawable) imageView.getDrawable();
        if (drawable == null) {
            MyLog.i("destroy item:recycle gif: gifdrawable is null");
        } else {
            MyLog.i("destroy item:recycle gif: gifdrawable is not null");
        }
        try {
            drawable.recycle();
        } catch (Exception e) {
...
//        MyLog.i("start gif");
        try {
            drawable.start();
//            MyLog.i("start gif isPlaying: " + drawable.isPlaying() + ", isRunning: " + drawable.isRunning());
        } catch (Exception e) {
            MyLog.e(e);
        }
    }
...
//        MyLog.i("stop gif");
        try {
            drawable.stop();
//            MyLog.i("stop gif isPlaying: " + drawable.isPlaying() + ", isRunning: " + drawable.isRunning());
        } catch (Exception e) {
            MyLog.e(e);
        }
    }
